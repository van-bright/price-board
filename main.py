...
def sell_order_quote(token_in, token_out, fee, price_decrease):
    ...
    target_price = curr_price - price_decrease
    target_sell_price = algo.price_to_sqrtp(target_price)  # sell eth make price decrease 300U
    ...
    return target_price, amount_in, max_output
...
if __name__ == '__main__':
    totalLiqudity = 0
    for i in range(10):
        p, output, _ = sell_order_quote(WETH_TOKEN, USDC_TOKEN, FeeAmount_Medium, 50 * i * one_usdc)

        print(f"price: {p/one_usdc}, liquidity: {(output)/one_eth}")
        totalLiqudity += output
